...
<head>
    ...
    <style>
        ...
        .gradient-text {
            background: linear-gradient(90deg, #1c09e3, #2d07c4);
            /* Gradient colors */
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Edit Invitational</title>
    <link rel="stylesheet" href="{{ mix('css/app.css') }}"> <!-- Ensure you have Tailwind CSS or your CSS file linked -->
</head>
<body>
    <div class="w-full min-h-screen flex items-center justify-center bg-gray-100">
        <div class="w-full h-full flex items-center justify-center">
            <div class="w-full sm:w-5/6 md:w-2/3 lg:w-1/2 xl:w-1/3 2xl:w-1/4 bg-white p-8 rounded shadow-md">
                <form method="POST" action="{{ route('invitational.update', $invitational->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex flex-col my-4">
                            ...
                            <select name="cluster" id="cluster" class="mt-2 p-2 border border-gray-300 focus:outline-none focus:ring-0 focus:border-gray-300 rounded text-xl text-gray-900">
                                ...
                                <option value="Cluster 1" {{ $invitational->cluster === 'Cluster 1' ? 'selected' : '' }}>Cluster 1</option>
                                <option value="Cluster 2" {{ $invitational->cluster === 'Cluster 2' ? 'selected' : '' }}>Cluster 2</option>
                                <option value="Cluster 3" {{ $invitational->cluster === 'Cluster 3' ? 'selected' : '' }}>Cluster 3</option>
                                <option value="Cluster 4" {{ $invitational->cluster === 'Cluster 4' ? 'selected' : '' }}>Cluster 4</option>
                                <option value="Cluster 5" {{ $invitational->cluster === 'Cluster 5' ? 'selected' : '' }}>Cluster 5</option>
                                <option value="Cluster 6" {{ $invitational->cluster === 'Cluster 6' ? 'selected' : '' }}>Cluster 6</option>
                                <option value="Cluster 7" {{ $invitational->cluster === 'Cluster 7' ? 'selected' : '' }}>Cluster 7</option>
                                <option value="Cluster 8" {{ $invitational->cluster === 'Cluster 8' ? 'selected' : '' }}>Cluster 8</option>
                                <option value="Cluster 9" {{ $invitational->cluster === 'Cluster 9' ? 'selected' : '' }}>Cluster 9</option>
                                <option value="Cluster 10" {{ $invitational->cluster === 'Cluster 10' ? 'selected' : '' }}>Cluster 10</option>
                                <option value="Cluster 11" {{ $invitational->cluster === 'Cluster 11' ? 'selected' : '' }}>Cluster 11</option>
                                <option value="LGU All Star" {{ $invitational->cluster === 'LGU All Star' ? 'selected' : '' }}>LGU All Star</option>
                            </select>
                        </div>
                        ...
                        <div class="flex flex-col my-4">
                            ...
                            @if($invitational->logo)
                                <img src="{{ asset('storage/' . $invitational->logo) }}" alt="Current Logo" class="mt-2 h-32 w-auto">
                            @endif
                        </div>
                    </div>
                        ...
                        <div class="space-y-4 mb-4">
                            @foreach([
                                ...
                                <div class="flex flex-col my-4">
                                    ...
                                    <input type="number" name="{{ $key }}" id="{{ $key }}" class="mt-2 p-2 border border-gray-300 focus:outline-none focus:ring-0 focus:border-gray-300 rounded text-xl text-gray-900" placeholder="Enter {{ $label }} Score" value="{{ old($key, $invitational->$key) }}">
                                </div>
                            @endforeach
                        </div>
